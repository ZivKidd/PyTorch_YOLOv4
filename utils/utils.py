...
def plot_gt_pre(images, targets_gt,targets_pre ,paths=None, fname='images.jpg',
                names=None, max_size=80000, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    ...
    if isinstance(targets_gt, torch.Tensor):
        targets_gt = targets_gt.cpu().numpy()

    if isinstance(targets_pre, torch.Tensor):
        targets_pre = targets_pre.cpu().numpy()
    ...
    for i, img in enumerate(images):
        ...
        if len(targets_gt) > 0:
            image_targets = targets_gt[targets_gt[:, 0] == i]
            ...
            for j, box in enumerate(boxes.T):
                ...
                label = ''
                plot_one_box(box, mosaic, label=label, color=color, line_thickness=tl+12)
                # # 如果是预测值
                # elif(conf[j]>0.3):
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=2560, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    if os.path.isfile(fname):  # do not overwrite
        return None
