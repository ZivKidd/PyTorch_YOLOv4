...
for ind,tag in enumerate(tqdm.tqdm(tag_files)):
    tiff = tag[:-4] + '.txt'
    if(ind%7<5):
        shutil.move(tag,os.path.join(train_folder,os.path.split(tag)[1]))
        shutil.move(tiff,os.path.join(train_folder,os.path.split(tiff)[1]))
    # elif(ind%7==4):
    #     shutil.copy(tag,os.path.join(val_folder,os.path.split(tag)[1]))
    #     shutil.copy(tiff,os.path.join(val_folder,os.path.split(tiff)[1]))
    else:
        shutil.move(tag,os.path.join(test_folder,os.path.split(tag)[1]))
        shutil.move(tiff,os.path.join(test_folder,os.path.split(tiff)[1]))
